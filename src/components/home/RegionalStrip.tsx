  ...
  return (
    <section className="bg-white py-16 sm:py-20">
      <div className="container-page">
        <SectionHeading
          eyebrow="Multi-country coverage"
          title="One eSIM for your whole trip"
          subtitle="Crossing borders? One plan covers every country in the region."
          action={
            <Link href="/regional-plans" className="inline-flex items-center gap-1 text-sm font-semibold text-orange hover:underline">
              See all regions <ArrowRight className="size-3.5" />
            </Link>
          }
        />
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-7">
          ...
          >
            ...
            <p className="text-sm font-semibold text-ink">Global</p>
            <p className="text-xs text-muted">{globalCountryCount}+ countries</p>
            <p className="text-xs font-semibold text-orange">from {formatPrice(globalFromPrice)}</p>
          </Link>
          ...
          {FEATURED_REGIONS.map(({ slug, icon, href }) => {
            ...
            return (
              ...
              >
                ...
                <p className="text-sm font-semibold text-ink">{region.name}</p>
                <p className="text-xs text-muted">{region.countryCount} countries</p>
                <p className="text-xs font-semibold text-orange">from {formatPrice(region.fromPrice)}</p>
              </Link>
            );
          })}
        </div>
